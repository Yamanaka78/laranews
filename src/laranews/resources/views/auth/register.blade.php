<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <a class="flex items-center justify-center text-gray-600 font-bold p-5 mb-4 md:mb-0" href="{{ route('top') }}">
                ...
                <!-- title -->
                <span class="ml-3 text-2xl">Laranews会員登録</span>
        </a>
        <!-- Name -->
        <div>
            <x-input-label for="name" :value="__('ユーザー名')" />
            <x-text-input id="name" class="block mt-5 w-full" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
        ...
        <div class="mt-4">
            ...
            <x-text-input id="email" class="block mt-5 w-full" type="email" name="email" :value="old('email')" required autocomplete="username" />
